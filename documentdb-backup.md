...
> On drivers: the `apis.Driver` enum is `Restic | WalG | Medusa | VolumeSnapshotter | Solr |
> ClickHouseBackup | Neo4jAdmin`. **There is no `Kopia` value** — KubeStash uses a
...
17 apps exist today: `cassandra clickhouse druid elasticsearch kubedbmanifest kubedbverifier mariadb
mongodb mssqlserver mysql neo4j opensearch postgres qdrant redis singlestore zookeeper`.
